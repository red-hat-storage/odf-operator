...
func ensureQuickStarts(ctx context.Context, cli client.Client, log logr.Logger) error {
	for _, qs := range AllQuickStarts {
		...
		if err != nil {
			log.Error(err, "failed to create or update quickstart", "Name", desiredCQS.Name, "Namespace", desiredCQS.Namespace)
			return nil
		}
		log.Info("updating quickstarts", "Name", desiredCQS.Name, "Namespace", desiredCQS.Namespace)
	}
	return nil
}
