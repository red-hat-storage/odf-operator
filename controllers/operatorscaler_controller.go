...
func (r *OperatorScalerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	resMappingList, err := GetCrdAndPackagesMappingList(ctx, r.Client, logger, r.OperatorNamespace)
	if err != nil {
		logger.Error(err, "failed to get resource mapping")
		return ctrl.Result{}, err
	}

	if err := r.reconcileOperators(ctx, logger, resMappingList); err != nil {
		return ctrl.Result{}, err
	}

	if err := r.reconcileDynamicWatchers(ctx, logger, resMappingList); err != nil {
		return ctrl.Result{}, err
	}
...
func (r *OperatorScalerReconciler) reconcileOperators(ctx context.Context, logger logr.Logger, resMappingList []CrdAndPackages) error {
	logger.Info("entering reconcileOperators")

	var returnErr error

	for i := range resMappingList {
		resourceMapping := &resMappingList[i]

		if !resourceMapping.CrdPresent || resourceMapping.ApiVersion == "" || resourceMapping.Kind == "" {
...
func (r *OperatorScalerReconciler) reconcileDynamicWatchers(ctx context.Context, logger logr.Logger, resMappingList []CrdAndPackages) error {
	logger.Info("entering reconcileDynamicWatchers")

	for i := range resMappingList {
		resourceMapping := &resMappingList[i]

		if !r.kindsBeingWatched[resourceMapping.Kind] && resourceMapping.CrdPresent {
		...
		Watches(
			...
			builder.WithPredicates(
				predicate.NewPredicateFuncs(func(obj client.Object) bool {
					ctx := context.TODO()
					crdMap, err := GetCrdNamesMap(ctx, r.Client, log.FromContext(ctx), r.OperatorNamespace)
					if err != nil {
						return false
					}
