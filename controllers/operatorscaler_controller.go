...
import (
	...
	oprv1a1 "github.com/operator-framework/api/pkg/operators/v1alpha1"
...
func (r *OperatorScalerReconciler) isOdfDependenciesCsvReady() error {
	...
	odfDepsCsv := &oprv1a1.ClusterServiceVersion{}
	...
	if odfDepsCsv.Status.Phase != oprv1a1.CSVPhaseSucceeded {
...
func (r *OperatorScalerReconciler) reconcileOperators() error {
	...
	for i := range ResourceMappingList {
		...
		err := r.Client.List(r.ctx, crList, client.Limit(1))
		if err != nil {
			...
			for _, pkgName := range resourceMapping.PkgNames {
				...
				csvList := &oprv1a1.ClusterServiceVersionList{}
				err = r.Client.List(
					r.ctx, csvList,
				...
				if err != nil {
					...
					err = r.updateCsvDeplymentsReplicas(csvList.Items...)
					if err != nil {
...
func (r *OperatorScalerReconciler) updateCsvDeplymentsReplicas(csvList ...oprv1a1.ClusterServiceVersion) error {

	var returnErr error

	for idx := range csvList {
		csv := &csvList[idx]
		...
		for i := range csv.Spec.InstallStrategy.StrategySpec.DeploymentSpecs {
			...
			if *deploymentSpec.Replicas < int32(1) {
...
func (r *OperatorScalerReconciler) reconcileDynamicWatchers() error {
	...
	for i := range ResourceMappingList {
		...
		if !r.kindsBeingWatched[resourceMapping.Kind] {
			...
			if err := r.Client.Get(r.ctx, types.NamespacedName{Name: resourceMapping.CrdName}, crd); err != nil {
		...
		Watches(
			...
			builder.WithPredicates(
				predicate.NewPredicateFuncs(func(obj client.Object) bool {
					return slices.ContainsFunc(ResourceMappingList, func(resourceMapping ResourceMappingRecord) bool {
						return resourceMapping.CrdName == obj.GetName()
					})
				}),
