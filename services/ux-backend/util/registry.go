...
func getRegistryCredentials(secretName string, secretNamespace string, client client.Client) (*types.DockerAuthConfig, error) {
	secret := &corev1.Secret{}
	if err := client.Get(context.Background(), k8stypes.NamespacedName{Name: secretName, Namespace: secretNamespace}, secret); err != nil {
...
func TestRegistryConnection(registryURL string, registryRepositoryName string, secretKey string, secretNamespace string, client client.Client) error {
	...
	credentials, err := getRegistryCredentials(secretKey, secretNamespace, client)
	if err != nil {
	...
	image, err := ref.NewImageSource(context.TODO(), sys)
	if err != nil {
		klog.Errorf("failed to get image: %v", err)
		return fmt.Errorf("failed to get image: %v", err)
	}
	defer image.Close()
	_, _, err = image.GetManifest(context.TODO(), nil)
	if err != nil {
